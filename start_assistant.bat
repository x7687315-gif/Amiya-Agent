@echo off
REM =====================================================================
REM  M4: start_assistant.bat  —  一键启动 阿米娅 Agent + 阿米娅语音
...
REM  【编码约束】本文件必须保存为 ANSI/GBK 编码。不要改成 UTF-8，
REM  也不要加 chcp 65001 —— cmd 解析中途切代码页会错位执行 REM 片段（已实测）。
...
set "TTS_PORT=9880"
REM  【同步提醒】TTS_HOST/TTS_PORT 必须与 core\tts\voice_profiles\assistant.yaml
REM             的 api.host/port 保持一致（Agent 按该 yaml 连接 TTS）。
set "TIMEOUT_SEC=600"
